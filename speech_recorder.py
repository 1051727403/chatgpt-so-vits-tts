...
def record_wave(wavfile,
    ...
    print('Start to record with {}-seconds audio\n'
          ...
          .format(duration))
    length = int(sampling_rate / chunk_size * duration)
    perlength = int(length/duration)
    try:
        for _ in range(0, length):
            data = stream.read(chunk_size)
            frames.append(data)
            print(_)
            if _==length/2: prompt+='\n'
            elif _%perlength==0:prompt+=str(int((length-_)/perlength))
            else: prompt+='.'
            print('.', end='', flush=True)
            updateLabel.emit(prompt,'')
        prompt += 'end'
        updateLabel.emit(prompt,'')
